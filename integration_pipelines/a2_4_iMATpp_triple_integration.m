...
% % %% check fit of coupling and change of flux 
% load('output/integration_output/myCSM_exp_resp.mat')
% myCSM_merged = myCSM_exp_resp;
% a = table(myCSM_merged.OFD, myCSM.OFD);
...
% abs(MetFlux)' * weight
% myCSM.minMetBalanceLoss * 1.05
% 
% %% check all cases 
% diary output/integration_output/DEsimilarity_integration_checkpoints.txt
% 
...
% diary off
% %% check by metabolite
% met = 'malcoa[c]'; % 'accoa[c]' leu-L
% tbl3 = listRxn(model,myCSM_triple.OFD,met); % myCSM_merged.OFD
% tbl4 = listRxn(model,myCSM_merged.OFD,met); % myCSM_merged.OFD
% 

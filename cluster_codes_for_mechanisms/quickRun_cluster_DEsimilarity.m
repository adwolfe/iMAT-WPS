...
splits = 1:length(simi_depend);
for i = 1:length(splits)
    batchID = ['b_',uid,'_',num2str(i)];
    env_cmd = 'module load gurobi/10.0.0 && module load matlab/r2022b && ';
    cmd = ['matlab -nodisplay -nosplash -nojvm -r \"cluster_a2_prediction_mechanism_analysis_simi ',tmpDir,' ',num2str(splits(i)),'\"'];
    full_cmd = [env_cmd, cmd, ' > ',tmpDir,'/',batchID,'.log && wait'];
    bsub_cmd = ['bsub -q long -W 6:00 -n 1 -R rusage[mem=4096] -e ',tmpDir,'/err_',batchID,'.log ','-J ',batchID,' "'];
...
for i = 1:length(splits)
    ...
    predTbl(strcmp(predTbl.rxns, simi_depend{i}),'related_similarity_constraints') = tmp(strcmp(tmp.rxns, simi_depend{i}),"related_similarity_constraints");
end

writetable(predTbl,'output/mechanism_analysis/fluxTable_ann_similarity.csv')

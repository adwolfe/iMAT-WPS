...
yH = logical(solution.int(1:length(RHindex)) + solution.int((length(RHindex)+length(RLindex)+1):(2*length(RHindex)+length(RLindex))));
nameH = model.rxns(ismember(model.rxns,RHNames));
OpenedHReaction = nameH(yH);
nameL = model.rxns(ismember(model.rxns,RLNames));
yL = logical(solution.int((length(RHindex)+1):(length(RHindex)+length(RLindex))));
ClosedLReaction = nameL(yL);
OpenGene = HGeneList(logical(solution.int((2*length(RHindex)+length(RLindex))+1:end)));
end

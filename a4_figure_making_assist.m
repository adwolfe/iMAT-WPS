...
% if a flux is too small that log scale is negative, the arrow width is
% 0.25
%% load
load('output/integration_output/myCSM_exp_resp_simi.mat')
load('./input/model/makeWormModel/iCEL1314_withUptakes.mat');
flux_triple = myCSM_exp_resp_simi.OFD ./ abs(myCSM_exp_resp_simi.OFD(strcmp(model.rxns,'EXC0050')));
annTbl = readtable('output/fluxTable.csv', 'ReadRowNames',true);

%% For most arrows, we calculate by checking the metabolite of interest shown in the map
met = 'ipdp[c]'; % 'accoa[c]' leu-L
...
tbl3.confidence_PFD = annTbl.PFD_bounded_exp_resp_simi(tbl3.tbl31)
...
%% succoa to succ total flux 
rxns = {'RM00432', 'RM00405'}; % one through atp one through gtp
flux = sum(abs(flux_triple(ismember(model.rxns,rxns))));
len = 0.5 * log2(200 * flux)
%% TCA intermediate transports
% we skipped the visualization of the flux shuffling TCA intermediates
% (oaa, cit, icit, asp, glu, mal) to support cytosolic citrate lyase,
% because of crowded space 
 
%% CYS-L FROM AA FOOD
tbl3 = listRxn(model,flux_triple,'cys-L[c]');
tbl3 = cell2table(tbl3);
flux = abs(tbl3.tbl35(strcmp('RC00893', tbl3.tbl31))) - abs(tbl3.tbl35(strcmp('RC01001', tbl3.tbl31))); % ignore all small fluxes (<10% of the largest)
len = 0.5 * log2(200 * flux)
%% beta oxidation - accoa 
rxns = {'RM04747','RM01177','RMC0107','RM03778','RM04742','RM03858','RMC0090','RMC0088','RMC0106'};
...
len = 0.5 * log2(200 * flux)
% most of the major effluxes are bounded, so we used bounded 
...
tbl3 = listRxn(model,flux_triple,'gly[m]');
tbl3 = cell2table(tbl3);
...
len = 0.5 * log2(200 * flux)
% the top catabolic fluxes are bounded so we put bounded arrow for this

%% lysine for deg
tbl3 = listRxn(model,flux_triple,'lys-L[c]');
tbl3 = cell2table(tbl3);
flux = flux_triple(strcmp(model.rxns,'TCM0089')) - sum(abs(flux_triple(ismember(model.rxns,{'RM03658'}))));
len = 0.5 * log2(200 * flux)
%% tyrosine for deg
tbl3 = listRxn(model,flux_triple,'tyr-L[c]');
tbl3 = cell2table(tbl3);
flux = abs(flux_triple(strcmp(model.rxns,'TCM1082'))) + (abs(flux_triple(ismember(model.rxns,{'RC00734'})))) + (abs(flux_triple(ismember(model.rxns,{'RC00736'})))) - (abs(flux_triple(ismember(model.rxns,{'RM02918'}))));
len = 0.5 * log2(200 * flux)

%% phe to tyrosine
tbl3 = listRxn(model,flux_triple,'phe-L[c]');
tbl3 = cell2table(tbl3);
flux = abs(flux_triple(strcmp(model.rxns,'RC01795'))) + (abs(flux_triple(ismember(model.rxns,{'RC07211'}))));
len = 0.5 * log2(200 * flux)

%% total 34hpp in tyr deg
tbl3 = listRxn(model,flux_triple,'34hpp[c]');
tbl3 = cell2table(tbl3);
flux = abs(flux_triple(strcmp(model.rxns,'RC02521'))) + (abs(flux_triple(ismember(model.rxns,{'RC03336'}))));
len = 0.5 * log2(200 * flux)
% the major flux 'RC03336' is bounded so we consider it bounded 

%% trp deg
% from trp to 2oxoadp: most reactions are bounded, so we consider it
% bounded 
...
met = ['nadph[c]']; % 'accoa[c]' leu-L
tbl3 = listRxn(model,flux_triple,met); % myCSM_merged.OFD
tbl3 = cell2table(tbl3);
tbl3.length = 3 * abs(tbl3.tbl35); % anchor 0.01 at 0.5 with an increment of 0.5 per 2-fold increase
%% nadph balance 1
% nonPPPrxns = {'RC08759','RC02236','RC08383','RC00941'}; manually select
% nonPPP rxns from the table 
...
met = 'etfrd[m]'; % 'accoa[c]' leu-L
tbl3 = listRxn(model,flux_triple,met); % myCSM_merged.OFD
tbl3 = cell2table(tbl3);
tbl3.length = abs(tbl3.tbl35)*0.75; % 
tbl3.confidence_PFD = annTbl.PFD_bounded_exp_resp_simi(tbl3.tbl31)

%% etfrd balance
AAdeg = {'RM04432','RM02661','RM04096','RM03172','RM02488'};
fadeg = {'RM04751','RM03777','RM01175','RM04754','RM03857','RM03990'};
tbl3 = listRxn(model,flux_triple,'etfrd[m]');
tbl3 = cell2table(tbl3);
AAdeg_len = sum(abs(tbl3.tbl35(ismember(tbl3.tbl31, AAdeg))))*0.75
fadeg_len = sum(abs(tbl3.tbl35(ismember(tbl3.tbl31, fadeg))))*0.75
fadh2deg_len = sum(abs(tbl3.tbl35(ismember(tbl3.tbl31, {'RMC0155'}))))*0.75

%% fadh2 from FA deg
rxns = {'RMC0106','RMC0107','RMC0090','RMC0088'};
tbl3 = listRxn(model,flux_triple,'fadh2[m]');
tbl3 = cell2table(tbl3);
f1 = sum(abs(tbl3.tbl35(ismember(tbl3.tbl31, rxns))))*0.75
...
others = (sum(abs(tbl3.tbl35)) / 2 - 2*(len1 + len2 + len3 + len4 + len5))/2
